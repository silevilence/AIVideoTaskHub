...
- [x] **[P2] MCP服务器支持**
    - [x] 系统设置模块新增 MCP 服务端启停控制开关，统筹管理下游服务进程的生命周期
    - [x] 引入 MCP 官方 TypeScript SDK 进行底层架构重构，并结合 streamableHttp 协议构建通信链路，保障数据交互的标准化与稳定性
    - [x] 实例化并启动 MCP 服务端进程，对外暴露标准化的工具调用接口（Tool Calling API）
    - [x] 基于 SDK 规范，封装并注册以下核心业务工具集（按标准业务流转链路排序）：
        - [x] 获取模型列表：聚合查询所有供应商当前支持的可用模型矩阵
        - [x] 获取参数规范：查询指定供应商及模型对应的前置入参约束与配置协议
        - [x] 提交生成任务：基于 Prompt、参考图像（兼容 Base64 与 URL 协议）、供应商 ID、模型 ID 及动态参数集（封装于 `"params":{}` 内，且需与入参规范严格校验映射）触发异步生成作业，并同步下发全局唯一任务 ID
        - [x] 查询任务大盘：批量检索全局任务列表，返回核心字段需涵盖任务 ID、实时运行状态、承载模型及 Prompt 摘要（前 50 字符截断）
        - [x] 查询任务详情：基于目标任务 ID 进行精确穿透，获取该任务的完整生命周期节点状态及全量入参快照
        - [x] 提取视频资产：基于任务 ID 换取最终视频产物的下载链接（内置状态屏障机制：仅当任务流转至“已完成”状态且资产就绪时，方可下发有效 URL，否则阻断请求并抛出标准化的业务异常提示）

- [x] **[P2] 提示词优化功能支持参考图**
    - [x] 系统配置模块升级
        - [x] 模型属性扩充：新增「视觉能力（Vision）」标识，用于判定该模型是否原生支持多模态图像输入
        - [x] 全局默认项增设：新增「默认图像解析模型」配置项（非必填），该选项的数据源需受「视觉能力」标识约束，仅展示支持图文多模态的模型
    - [x] 基于目标模型视觉能力的双轨路由策略
        - [x] 策略 A：目标模型具备原生视觉能力
            - 采用直通模式（Pass-through），将用户上传的参考图像载荷与提示词直接下发至目标模型
        - [x] 策略 B：目标模型无原生视觉能力（前置解析降级方案）
            - UI 层动态渲染：交互界面按需展示「图像解析模型」选择器，并自动回显系统级的「默认图像解析模型」
            - 选项过滤机制：该选择器的下拉数据源需强制校验，仅允许选择挂载了「视觉能力」标识的模型
            - 任务流编排：引入前置图像解析链路，先将图片路由至解析模型生成语义描述（Captioning），随后将解析产物按标准化结构（如：`参考图[N]：[语义描述]`）无缝拼接到原始 Prompt 中，最终投递给目标优化模型
            - 边界异常兜底：若未配置或未选择图像解析模型，系统需触发强警告拦截；若用户授权强制放行，则在最终请求中剥离图像二进制/URL载荷（降级为纯文本请求），但必须在 Prompt 中注入占位声明，明确告知目标模型"存在 N 张参考图，但受限无法提供具体视觉输入"

- [x] **建立 ComfyUI 工作流模板数据与校验能力**
    - [x] 支持以稳定 ID 管理模板，并将 YAML 头元数据与纯 API 格式 JSON 作为组合文档持久化到 SQLite
    - [x] 支持模板列表、查询、新建、更新、复制、启停、删除以及单模板导入导出
    - [x] 校验 API 格式、变量 schema、变量引用、主描述和主输出，并区分阻止保存的错误与可确认的未使用变量警告
    - 验收：内存数据库与 API 测试覆盖全部生命周期；组合文档可无损导入导出；非法模板不能保存，只有未使用变量可确认后保存

- [x] **实现设置中的 ComfyUI 工作流管理器**
    - [x] 提供模板搜索、新建、编辑、复制、启停、删除、导入和导出交互
    - [x] 将变量、主描述和主输出显示为独立管理面板，JSON 区域仅显示纯工作流正文
    - [x] JSON 编辑器提供语法高亮、格式化、行列错误提示和 UI 工作流格式误用提示
    - [x] 支持连接默认 ComfyUI 检查节点兼容性，但在线检查失败不阻止离线保存
    - 验收：管理器可完整维护一份组合文档；错误、警告和在线兼容性结果均能准确定位，且已启用状态与模型可见性一致

- [x] **注册 ComfyUI Provider 与动态模板模型**
    - [x] 将已启用工作流模板作为 ComfyUI 模型返回，使用稳定 ID 并展示模板名称
    - [x] Provider 设置支持默认 HTTP/HTTPS Base URL、规范化保存和无认证连通性测试
    - [x] 对外提供每个模板的动态变量 schema、主描述和主输出信息，供 Web 与 MCP 共用
    - 验收：模板新增、改名、启停和删除会正确反映到模型与参数协议；临时地址不会覆盖默认设置；其他 Provider 的模型与设置行为不变

- [x] **构建 ComfyUI 动态任务表单与模板渲染**
    - [x] 选择模板后按定义顺序渲染整数、小数、字符串、布尔、选项和图片字段，并自动写入默认值
    - [x] 创建页支持本次临时覆盖 ComfyUI 地址，并在无可用模板时引导进入工作流管理
    - [x] 前后端共同校验所有变量已填写且满足类型、范围、长度和选项约束
    - [x] 以类型安全方式替换 `inputs` 中的合法 JSON 字符串令牌，禁止结构位置变量和非字符串内嵌插值
    - 验收：各变量控件、默认值和错误提示符合模板定义；合法输入渲染出可解析 API 工作流，恶意或非法输入不能改变工作流结构

- [x] **实现 ComfyUI 实际地址强制预检、图片上传与任务快照**
    - [x] 每次创建任务前按本次实际地址读取 `/object_info`，检查所需节点与可识别输入兼容性
    - [x] 地址不可达、检查失败或节点不兼容时阻止本地任务创建与远端排队，并返回完整问题清单
    - [x] 图片变量支持本地上传、现有图片库和 URL，统一上传到目标 ComfyUI 后注入返回的文件标识
    - [x] 创建任务时保存模板文档、变量值、图片解析结果、主输出和实际地址的完整快照
    - 验收：临时地址必须独立预检且不能误用其他实例结果；预检失败不产生任务；模板随后变更或删除时任务快照仍完整可读

- [x] **接通 ComfyUI 异步执行与单视频结果下载**
    - [x] 通过 `/prompt` 提交渲染后的工作流，保存 `prompt_id` 并呈现节点校验错误
    - [x] 通过任务快照地址查询队列与历史，将远端状态映射到现有四态生命周期
    - [x] 按主输出节点、字段和序号定位标准文件描述，并将一个视频下载到现有本地资源目录
    - [x] 对执行失败、输出缺失、文件描述异常和下载失败记录可诊断错误
    - 验收：模拟 ComfyUI 服务覆盖排队、执行、成功、失败和下载路径；多输出工作流只保存声明的主输出；现有轮询器回归测试通过

- [x] **兼容 ComfyUI 历史任务、套用参数与 MCP**
    - [x] 任务详情与回收站展示模板、变量、主输出、实际地址和图片预览信息
    - [x] 重试和套用参数使用历史任务快照，并在原模板停用或删除后仍可工作；重试再次执行实际地址预检
    - [x] MCP 模型查询列出已启用模板，参数协议返回变量 schema，任务提交接受变量值和临时地址并复用相同后端校验
    - [x] 保持任务删除、媒体清理和其他 Provider 的重试、套用与 MCP 行为不变
    - 验收：模板变更、停用和删除场景均有 API/UI/MCP 回归测试；历史快照可创建等价新任务，且其他 Provider 全量测试通过
