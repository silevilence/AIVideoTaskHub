...
- [ ] **注册 ComfyUI Provider 与动态模板模型**
    - [ ] 将已启用工作流模板作为 ComfyUI 模型返回，使用稳定 ID 并展示模板名称
    - [ ] Provider 设置支持默认 HTTP/HTTPS Base URL、规范化保存和无认证连通性测试
    - [ ] 对外提供每个模板的动态变量 schema、主描述和主输出信息，供 Web 与 MCP 共用
