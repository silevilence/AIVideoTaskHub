...
- [ ] **建立 ComfyUI 工作流模板数据与校验能力**
    - [ ] 支持以稳定 ID 管理模板，并将 YAML 头元数据与纯 API 格式 JSON 作为组合文档持久化到 SQLite
    - [ ] 支持模板列表、查询、新建、更新、复制、启停、删除以及单模板导入导出
    - [ ] 校验 API 格式、变量 schema、变量引用、主描述和主输出，并区分阻止保存的错误与可确认的未使用变量警告
