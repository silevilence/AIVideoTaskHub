...
- [ ] **[P2] 用户体验增强**
    ...
    - [ ] 任务参数套用功能
        - [ ] 查看任务参数详情时（包括回收站），点击套用此参数，转到创建任务界面并套用相应的参数
        - [ ] 在创建任务界面，点击套用参数按钮，弹窗显示当前任务列表（可以点击查看参数详情、预览视频，但不能下载视频和删除任务），选择一个任务套用其参数（或者取消不套用）；可以切换显示任务列表或回收站任务列表
        - [ ] 套用参数时，如果有参考图，为链接的套用链接，为本地图片的不要重复上传，直接使用当时上传的图片
