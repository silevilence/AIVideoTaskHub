...
- [ ] **实现设置中的 ComfyUI 工作流管理器**
    - [ ] 提供模板搜索、新建、编辑、复制、启停、删除、导入和导出交互
    - [ ] 将变量、主描述和主输出显示为独立管理面板，JSON 区域仅显示纯工作流正文
    - [ ] JSON 编辑器提供语法高亮、格式化、行列错误提示和 UI 工作流格式误用提示
    - [ ] 支持连接默认 ComfyUI 检查节点兼容性，但在线检查失败不阻止离线保存
