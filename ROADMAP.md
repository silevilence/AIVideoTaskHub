...
- [x] **[P2] 提示词优化功能**
    - [x] **1. 设置模块重构与文本提供商配置**（底层支持）
        - [x] 重构设置界面布局：拆分为「视频设置」（承接原有全部设置）与「文本设置」两大独立页签
        - [x] 实现文本提供商（Provider）数据结构：包含名称（全局唯一）、Base URL、API Key、模型列表四项基础字段
        - [x] 预置主流提供商及其默认 Base URL：DeepSeek、硅基流动、火山引擎、AIHubMix（额外增加 `app code` 字段支持）
        - [x] 支持用户自定义添加提供商：兼容 OpenAI 接口规范的提供商、Ollama 本地服务（配置存本地）
        - [x] 实现 API Key 的智能复用与隔离机制：
            - [x] 文本提供商与视频提供商重合时，支持一键“使用视频设置中的 API Key”或“独立输入新 Key”
            - [x] 确保两套逻辑的数据独立保存，互不干扰
        - [x] 模型获取与管理逻辑：
            - [x] 支持通过 OpenAI 标准接口拉取模型列表，或由用户手动输入模型信息
            - [x] 模型属性配置：支持设置模型 ID（接口调用参数）、展示名称、是否具备思考（Reasoning）能力
            - [x] 存储优化：仅持久化保存用户最终选择的模型配置，不全量缓存获取到的整个模型列表
            - [x] 功能边界限制：当前版本仅限文本处理，暂不暴露多模态相关入口或参数
        - [x] 全局生成偏好：支持设置开启/关闭「流式输出」模式
        - [x] 支持设置Prompt输出语言
            - 有一个全局的默认设置
            - 各视频模型可以单独设置使用的语言
            - 影响到`${lang}`占位符的替换结果
    - [x] **2. 优化 Prompt 配置模块**（逻辑支持）
        - [x] 预置一套系统默认的提示词优化 Prompt（见下）
        - [x] 在「文本设置」界面中新增 Prompt 自定义编辑区域
            - 用户保存自定义模板时，检查必须要有`${input}`占位符
            - 用户保存自定义模板时，检查没有`${lang}`占位符时提出警告
        - [x] 提供「恢复默认」按钮，支持一键重置为系统预置 Prompt
        - [x] 支持`${input}`占位符，表示用户的原始输入
        - [x] 支持`${lang}`占位符，表示输出的语言
    - [x] **3. UI 交互与功能实现**（视图与功能）
        - [x] 入口改造：在现有的提示词输入区域新增「AI 优化」操作按钮
        - [x] 优化工作台（弹窗 Modal）整体布局设计：
            - [x] 顶部控制栏：提供快速下拉切换文本模型的能力
            - [x] 核心内容区：划分为「输入区」与「结果区」
        - [x] 交互流转逻辑实现：
            - [x] 上下文模式：当前版本限制为单轮会话，每次基于输入区最新内容独立触发生成
            - [x] 状态控制：支持在生成过程中「中断」、「重新生成」、「切换模型后重新生成」
            - [x] 数据填充机制：打开弹窗时「输入区」默认带入用户的原始提示词；生成完毕后使用 LLM 响应内容填充「结果区」
            - [x] 用户手动干预：允许用户直接在「输入区」或「结果区」进行手动文本修改
            - [x] 边界情况处理（拦截保存）：当用户放弃采用结果（关闭弹窗），但检测到「输入区」内容已发生变更时，触发二次确认弹窗，询问是否用更改后的输入内容覆盖原输入框数据
```text
...
- [x] **[P2] 参数持久化与用户体验优化**
    - [x] **参数状态保持**：切换模型或服务提供商时，确保已上传的图片资源不丢失，实现跨模型的图片状态持久化
    - [x] **扩展参数状态保持**：对时长、分辨率、宽高比、种子值等配置项，在条件允许时同步实现状态保持；对目标模型不支持的参数，在内存中暂存其值，待切回兼容模型时自动恢复
        - 适用场景示例：由图生视频模型切换至文生视频模型时，图片参数于后台暂存；再次切换至支持图生视频的模型时，自动还原原始图片及相关配置
    - [x] **Prompt 编辑器增强**：在文本设置面板的 Prompt 输入区域内，集成 Markdown 语法高亮渲染能力
    - [x] **API 接入方式可配置化**
        - [x] 在视频设置面板中，为 AihubMix 提供商新增接入方式切换选项，支持在 OpenAI SDK 接入与现有直接接入两种模式间自由切换
    - [x] **服务健康检查（Health Check）**
...
- [x] **[P2] Docker Hub 部署优化与 Prompt 模块增强**
    - [x] 镜像发布流水线优化：触发自动推送至 Docker Hub 时，同步更新最新 README 文档至 Overview 页面
    - [x] Prompt 核心能力升级（演进为独立 Prompt 管理模块）
        - [x] 统一数据模型：规范 Prompt 基础属性，包含名称、正文内容及标签（Tag）
        - [x] 系统预置能力：内置标准化的默认提示词库
        - [x] 自定义生命周期管理：支持用户对自定义 Prompt 进行增、删、改操作
            - [x] 校验逻辑兼容：维持原有合法性检查规则不变
            - [x] 权限隔离控制：系统内置 Prompt 强制锁定（限制修改与删除）
        - [x] 资产组织与分类：支持用户创建目录树对 Prompt 进行分组管理
        - [x] 全局配置管理：支持在系统设置中指定全局默认应用的 Prompt
        - [x] 前端交互优化：在业务界面的 Prompt 调优入口，提供已有提示词库的快速检索与无缝切换
