...
- [ ] **构建 ComfyUI 动态任务表单与模板渲染**
    - [ ] 选择模板后按定义顺序渲染整数、小数、字符串、布尔、选项和图片字段，并自动写入默认值
    - [ ] 创建页支持本次临时覆盖 ComfyUI 地址，并在无可用模板时引导进入工作流管理
    - [ ] 前后端共同校验所有变量已填写且满足类型、范围、长度和选项约束
    - [ ] 以类型安全方式替换 `inputs` 中的合法 JSON 字符串令牌，禁止结构位置变量和非字符串内嵌插值
