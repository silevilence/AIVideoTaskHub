...
- [ ] **实现 ComfyUI 实际地址强制预检、图片上传与任务快照**
    - [ ] 每次创建任务前按本次实际地址读取 `/object_info`，检查所需节点与可识别输入兼容性
    - [ ] 地址不可达、检查失败或节点不兼容时阻止本地任务创建与远端排队，并返回完整问题清单
    - [ ] 图片变量支持本地上传、现有图片库和 URL，统一上传到目标 ComfyUI 后注入返回的文件标识
    - [ ] 创建任务时保存模板文档、变量值、图片解析结果、主输出和实际地址的完整快照
