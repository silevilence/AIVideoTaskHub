...
- [ ] **兼容 ComfyUI 历史任务、套用参数与 MCP**
    - [ ] 任务详情与回收站展示模板、变量、主输出、实际地址和图片预览信息
    - [ ] 重试和套用参数使用历史任务快照，并在原模板停用或删除后仍可工作；重试再次执行实际地址预检
    - [ ] MCP 模型查询列出已启用模板，参数协议返回变量 schema，任务提交接受变量值和临时地址并复用相同后端校验
    - [ ] 保持任务删除、媒体清理和其他 Provider 的重试、套用与 MCP 行为不变
