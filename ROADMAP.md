...
- [ ] **接通 ComfyUI 异步执行与单视频结果下载**
    - [ ] 通过 `/prompt` 提交渲染后的工作流，保存 `prompt_id` 并呈现节点校验错误
    - [ ] 通过任务快照地址查询队列与历史，将远端状态映射到现有四态生命周期
    - [ ] 按主输出节点、字段和序号定位标准文件描述，并将一个视频下载到现有本地资源目录
    - [ ] 对执行失败、输出缺失、文件描述异常和下载失败记录可诊断错误
